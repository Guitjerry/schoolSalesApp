...
public class TmZuoyeServiceImpl implements TmZuoyeService {
    ...
    public List<ZuoyeVo> listAllZuoyeByAdmin(String kemuname, String banjiname, Integer userid){
        ...
        StringBuffer hql = new StringBuffer(" select new com.xiaoyuan.service.gateway.entity.ZuoyeVo(d.kemuid,d.id as zuoyeid,a.ID as banjiid, d.task,b.name as kemuname,a.name as banjiname) from  TmBanJi a, TmKemu b,TmZuoYe d where  a.id=d.banjiid and b.id=d.kemuid ");
    ...
    public List<ZuoyeVo> listAllZuoye(String kemuname,String banjiname,Integer userid) {
        List<ZuoyeVo> zuoyeVos = null;
        StringBuffer hql = new StringBuffer(" select new com.xiaoyuan.service.gateway.entity.ZuoyeVo(d.kemuid,d.id as zuoyeid,a.ID as banjiid, d.task,b.name as kemuname,a.name as banjiname) from  TmBanJi a, TmKemu b,TmUserClassKemu c,TmZuoYe d where a.id=c.classId and b.id=c.kemuId and a.id=d.banjiid  and c.kemuId>0 ");
        if(userid!=null&&userid>0){
            hql.append(" and c.userId=?1");
        }
        if(!StringUtils.isEmpty(kemuname)){
    ...
    public ZuoyeVo listAllZuoyeById(Integer zuoyeid){
        List<ZuoyeVo> zuoyeVos = null;
        StringBuffer hql = new StringBuffer(" select new com.xiaoyuan.service.gateway.entity.ZuoyeVo(d.kemuid,d.id as zuoyeid,a.ID as banjiid, d.task,b.name as kemuname,a.name as banjiname) from  TmBanJi a, TmKemu b,TmBanjiKemu c,TmZuoYe d where a.id=c.banjiid and b.id=c.kemuid and a.id=d.banjiid and b.id=d.kemuid");
